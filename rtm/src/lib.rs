...
impl HoleKind {
    /// The stable short label for this hole kind — the word the matrix and the emitted files both
    /// print as a status. Kept here so the gate report and `--emit` never drift apart.
...
/// One capability's committed matrix: the spec-id (the file stem `<spec-id>.md`) and the rendered
/// Markdown. A checked-in *generated* artifact — reviewed as a diff, regenerated by `rtm --emit`.
#[derive(Debug, Clone)]
...
/// Render one deterministic Markdown matrix per capability (spec-id) over the matrix's scenario
/// rows. Pure function of the matrix: capabilities are visited in sorted order, rows sorted by
/// (req-id, scenario-id), site lists sorted, so re-emitting identical inputs is byte-identical.
/// A capability with only dangling tags (no declared scenario) has no rows and so no file.
pub fn emit_matrices(matrix: &Matrix) -> Vec<EmittedMatrix> {
    use std::collections::BTreeMap;

    let mut holes_by_key: std::collections::HashMap<&Key, Vec<&'static str>> = Default::default();
    for hole in &matrix.holes {
        if let Some(key) = &hole.key {
            holes_by_key.entry(key).or_default().push(hole.kind.label());
        }
    }

    let mut by_spec: BTreeMap<&str, Vec<&Row>> = BTreeMap::new();
    for row in &matrix.rows {
        by_spec
            .entry(row.scenario.key.spec_id.as_str())
            .or_default()
            .push(row);
    }
    ...
    by_spec
        .into_iter()
        .map(|(spec_id, mut rows)| {
            rows.sort_by(|a, b| {
                (&a.scenario.key.req_id, &a.scenario.key.scenario_id)
                    .cmp(&(&b.scenario.key.req_id, &b.scenario.key.scenario_id))
            });
            EmittedMatrix {
                spec_id: spec_id.to_string(),
                contents: render_matrix(spec_id, &rows, &holes_by_key),
            }
        })
        .collect()
}
...
fn render_matrix(
    spec_id: &str,
    rows: &[&Row],
    holes_by_key: &std::collections::HashMap<&Key, Vec<&'static str>>,
) -> String {
    let mut out = String::new();
    out.push_str(
        "<!-- Generated by `rtm --emit`. Do not hand-edit — re-run rtm to regenerate. -->\n\n",
    );
    out.push_str(&format!("# Traceability matrix: {spec_id}\n\n"));
    out.push_str("| Requirement | Scenario | Form (scope/quant) | Realizing sites | Covering tests | Status |\n");
    out.push_str("| --- | --- | --- | --- | --- | --- |\n");

    for row in rows {
        let mut sites: Vec<&str> = row
            .realizations
            .iter()
            .map(|realization| realization.site.as_str())
            .collect();
        sites.sort_unstable();
        sites.dedup();
        ...
        let status = match holes_by_key.get(&row.scenario.key) {
            None => "ok".to_string(),
            Some(labels) => {
...
mod tests {
    ...
    #[test]
    fn emitting_twice_over_identical_inputs_is_byte_identical() {
        let matrix = build(
            &[
                scenario("runs", unit_example()),
                scenario("guards", component_invariant()),
            ],
            &[],
            &[tag("runs", unit_example()), tag("guards", component_invariant())],
            &[
                realization_at("runs", "Beta"),
                realization_at("runs", "Alpha"),
                realization_at("guards", "Gamma"),
            ],
            &[],
        );

        let first = emit_matrices(&matrix);
        let second = emit_matrices(&matrix);

        assert_eq!(first.len(), 1, "all scenarios share one spec-id");
        assert_eq!(first[0].spec_id, "spec");
        assert_eq!(first[0].contents, second[0].contents);
        // Sites are sorted, not insertion-ordered — the load-bearing property for a clean diff.
        assert!(first[0].contents.contains("Alpha, Beta"));
    }

    #[test]
    fn an_emitted_capability_with_a_hole_shows_the_hole_in_its_row() {
        let matrix = build(
            &[scenario("runs", unit_example())],
            &[],
            &[],
        ...
        let emitted = emit_matrices(&matrix);

        assert_eq!(emitted.len(), 1);
        assert!(
            emitted[0].contents.contains("uncovered"),
            "row should carry the uncovered status: {}",
            emitted[0].contents
        );
        assert!(
            !emitted[0].contents.contains("| ok |"),
            "a holed row is not ok: {}",
            emitted[0].contents
        );
    }

    #[test]
    fn each_capability_emits_its_own_file() {
        let alpha = Scenario {
            key: Key {
                spec_id: "alpha".into(),
                ..key("a-runs")
            },
            ..scenario("a-runs", unit_example())
        };
        let beta = Scenario {
            key: Key {
                spec_id: "beta".into(),
            ...
            ..scenario("b-runs", unit_example())
        };
        let matrix = build(&[alpha, beta], &[], &[], &[], &[]);

        let emitted = emit_matrices(&matrix);

        let ids: Vec<&str> = emitted.iter().map(|e| e.spec_id.as_str()).collect();
        assert_eq!(ids, vec!["alpha", "beta"], "capabilities emit sorted");
    }
