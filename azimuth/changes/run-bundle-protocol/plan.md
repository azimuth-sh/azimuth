# Plan: Define Provider-Neutral Run Bundles

- [x] Freeze the strict Run bundle, fingerprint, aggregation and correction authority.
- [x] Add and apply the five routine Run-protocol requirements.
- [x] Implement strict parsing, normalization, fingerprints and correction-set verification.
- [x] Add service-free Run verification and inspection commands.
- [ ] Build the standalone synthetic Run conformance experiment.
- [ ] Integrate the conformance gate and update public and agent guidance.
- [ ] Run complete component, experiment, release-isolation and composed-model audits.
- [ ] Record the accepted outcome and archive the change.
