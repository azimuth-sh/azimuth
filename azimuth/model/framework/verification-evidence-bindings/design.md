# Design: framework/verification-evidence-bindings

## Residue

`Scope:`, `Quantification:`, `Oracle:` and `Context:` are authored and unvalidated. Nothing
compares a declared form against the method it describes, so each is a self-declaration the machine
tier can only check for membership in a closed set.
...
part of its Claim predicate, so realization honesty rests entirely on review.
